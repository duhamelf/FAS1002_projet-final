...
# Add a task to run the download_data.R script at 9:00 AM every day
addTask(scheduler, "download_data", "/path/to/script/download_data.R", 
...
# fallait prendre fpath et non fname 

#  comment automatiser? comme ça ok 
...
#faire une fonction pour ce qui suit?? quel mot change mettre comme argument de ma fct, url, fname, 
...
df2 <- read_csv(file = path(base_path, fname2))
...
# comparer ces deux trucs dans if plus haut 
```
...
df2 <- as_tibble(df2)
#consigne, mais ecq ça change qqchose? pas besoin deja le cas pca read en tiblle 
```
...
#fonctionne pr df2! Marche pas pr df1 pcq il y a déjà des continents dans country pê?? extraire autrement, ou juste prendre tableau combiné plus bas? avec continent 
...
life_exp_complete <- df2 %>%
    ...
    filter(
        time_passed == max(time_passed)
    )

# fonctionne, mais on peut accélérer : 
